...
//public 
	...
	//File Options
	public string searchPattern = "*.mp3"; //search pattern used to find files
	//Output
...
//Protected	
	...
	protected void drawSearchMessage(){
		...
		if(tt>4)
			GUILayout.Button(".....");
		if(tt>5)
			GUILayout.Button("Esta");
		if(tt>6)
			GUILayout.Button("Tomando");
		if(tt>7)
			GUILayout.Button("mucho");
		if(tt>8)
			GUILayout.Button("tiempo");
		if(tt>9)
			GUILayout.Button(".....");
	}
	...
	public void getFileList(DirectoryInfo di){
		...
		int countGoodFilter = 0;
		for(int d=0;d<dia.Length;d++) {
			string dirname = dia[d].Name;
			if (!dirname.Contains(".")) {
				countGoodFilter++;
			}
		}
		directories = new DirectoryInformation[countGoodFilter];
		countGoodFilter = 0;
		for(int d=0;d<dia.Length;d++){
			if (dia[d].Name.Contains(".")) {
				continue;
			}
			if(directoryTexture)
				directories[countGoodFilter] = new DirectoryInformation(dia[d],directoryTexture);
			else
				directories[countGoodFilter] = new DirectoryInformation(dia[d]);
			countGoodFilter++;
		}
