...
//				break;
	...
	void Update()
	{
		if(KinectInitialized)
		{
			...
			if(timer <= 0 && CalibrationText != null && CalibrationText.text != "Calibrando... esperando usuario")
			{
			...
			if(Player1Calibrated)
			{
				...
				foreach(KinectGestures.GestureData gestureData in player1Gestures)
				{
					...
					else if(gestureData.progress >= 0.1f)
					{
						...
						if(percentClick != null && gestureData.gesture == KinectGestures.Gestures.Click)
						{
							if(gestureData.progress>0.3f) {
								string sGestureText = "Click " + (gestureData.progress * 100).ToString("0.0") + "%";
								percentClick.text = sGestureText;
							} else {
								percentClick.text = "";
							}
						}
