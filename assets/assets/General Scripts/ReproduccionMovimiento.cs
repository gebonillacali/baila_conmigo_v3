...
public class ReproduccionMovimiento : MonoBehaviour
{
...
//	public bool NearMode = false;
	...
	public static ReproduccionMovimiento instance;
...
//				break;
	...
	void Awake()
	{
		...
		if(CalibrationText != null)
		{
			CalibrationText.guiText.text = "Calibrando... esperando usuario";
			if(percentClick != null) {
				percentClick.text = "";
			}
			if(HandCursor1 != null) {
				HandCursor1.guiTexture.enabled = false;
			}
			timer = -1;
			Screen.showCursor = true;
			if(EjecucionRutina.initiated) {
				EjecucionRutina.paused =true;
				EjecucionRutina.pausado =true;
			}
	...
	void Update()
	{
		if( KinectWrapper.isPlaying()) {
			...
			if (KinectWrapper.PollSkeleton (ref smoothParameters, ref skeletonFrame, true)) {
					Debug.Log("processing skeleton");
					ProcessSkeleton ();
			}
    ...
    void CalibrateUser(uint UserId, int UserIndex, ref KinectWrapper.NuiSkeletonData skeletonData)
    {
		...
		if(AllPlayersCalibrated)
		{
			...
			if(CalibrationText != null)
			{
				...
				timer = 5;
				if(EjecucionRutina.initiated) {
					EjecucionRutina.paused =false;
					EjecucionRutina.pausado =false;
				}
			}
		}
    }
	...
	void ProcessSkeleton()
	{
		...
		for(int i = 0; i < KinectWrapper.Constants.NuiSkeletonCount; i++)
		{
			...
			if(skeletonData.eTrackingState == KinectWrapper.NuiSkeletonTrackingState.SkeletonTracked)
			{
				...
				Debug.Log(skeletonPos);
				Debug.Log(AllPlayersCalibrated);
				...
				//   (MaxUserDistance <= 0f || Mathf.Abs(skeletonPos.z) <= MaxUserDistance))
				{
					...
					for (int j = 0; j < (int)KinectWrapper.NuiSkeletonPositionIndex.Count; j++)
					{
						...
						//{
							Debug.Log("modificandoplayerjoints");
