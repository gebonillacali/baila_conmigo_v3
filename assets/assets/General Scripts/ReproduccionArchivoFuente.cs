...
//    }
	...
	public static bool PollSkeleton(ref NuiTransformSmoothParameters smoothParameters, ref NuiSkeletonFrame skeletonFrame)
	{
		...
		if(frameCount >= 10){
			frameCount = 0;
			curFrame++;
			newSkeleton = true && curFrame < (skeletonFrames.Length - 1);
			skeletonFrame = skeletonFrames[curFrame];
			return newSkeleton;
		}		
		return newSkeleton;
	}
	...
	public static bool PollColor(IntPtr colorStreamHandle, ref byte[] videoBuffer, ref Color32[] colorImage)
	{
		...
		int hr = ReproduccionArchivoFuente.NuiImageStreamGetNextFrame(colorStreamHandle, 0, ref imageFramePtr);
		if (hr == 0)
		{
			newColor = true;
			...
			hr = ReproduccionArchivoFuente.NuiImageStreamReleaseFrame(colorStreamHandle, imageFramePtr);
		}
#endif
	...
	public static bool PollDepth(IntPtr depthStreamHandle, bool isNearMode, ref ushort[] depthPlayerData)
	{
		...
		if (isNearMode)
		{
			ReproduccionArchivoFuente.NuiImageStreamSetImageFrameFlags(depthStreamHandle, NuiImageStreamFlags.EnableNearMode);
		}
		else
		{
			ReproduccionArchivoFuente.NuiImageStreamSetImageFrameFlags(depthStreamHandle, NuiImageStreamFlags.None);
		}
		
		int hr = ReproduccionArchivoFuente.NuiImageStreamGetNextFrame(depthStreamHandle, 0, ref imageFramePtr);
		if (hr == 0)
		{
			newDepth = true;
			...
			frameTexture.UnlockRect(0);
			hr = ReproduccionArchivoFuente.NuiImageStreamReleaseFrame(depthStreamHandle, imageFramePtr);
		}
#endif
