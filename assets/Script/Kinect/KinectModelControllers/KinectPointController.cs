		...
		if (sw.pollSkeleton())
		{
			for( int ii = 0; ii < (int)Kinect.NuiSkeletonPositionIndex.Count; ii++) {
				...
				if( ((uint)Mask & (uint)(1 << ii) ) > 0 ){
					...
					Debug.Log("sw.bonePos[player,ii].x:"+sw.bonePos[player,ii].x);
					Debug.Log("sw.bonePos[player,ii].y:"+sw.bonePos[player,ii].y);
					Debug.Log("sw.bonePos[player,ii].z:"+sw.bonePos[player,ii].z);
